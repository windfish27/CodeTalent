...
(function($) {
    ...
    $.widget('ui.rangeslider', $.ui.mouse, {
        ...
        // add handle
        addHandle: function(handle, typename) {
            var self = this, o = this.options;
            // add handle to handle list
            o.handles.push(handle);

            o.typeNames.push(typename);
            // refresh handles
            this._refreshHandles();
            // refresh values
            this._refreshValue();
        ...
        _refreshHandles: function() {
            ...
            $('.ui-slider-range-text', this.element).remove();
            ...
            if (o.handles && o.handles.length) {                
                ...
                $.each(o.handles, function(i) {
                    ...
                    /*if (i < o.handles.length) {
                        ...
                        // add rangetext
                        var rangetext = $('<div><h2></h2><p></p></div>');
                        rangetext.appendTo(elem).addClass('ui-slider-range-text');
                        
                        // if handle type was set
                        if (o.handles[i].type !== null) {                        
                            range.attr('data-type', o.handles[i].type)
                            .addClass(o.handles[i].type);
                            rangetext.attr('data-type', o.handles[i].type).addClass(o.handles[i].type);
                            //rangetext.children("p").text(o.typeNames[o.handles[i]]);
                        }
                    }*/
            ...
            if(o.showTypeNames){
               for(i = 0; i < o.typeNames.length; i++){
                    var range = $('<div></div>');
                    ...
                    // add rangetext
                    var rangetext = $('<div><h2></h2><p></p></div>');
                    rangetext.appendTo(element).addClass('ui-slider-range-text');

                    range.attr({'data-type': o.typeNames[i].type,'data-value': i-1}).addClass(o.typeNames[i].type);
                    rangetext.attr({'data-type': o.typeNames[i].type,'data-value': i-1}).addClass(o.typeNames[i].type);
                    rangetext.children("p").text(o.typeNames[i].text);
                } 
            }
        ...
        removeHandle: function(index) {
            ...
            // from type names
            this.options.typeNames.splice(index, 1);
            // remove element
        ...
        _refreshValue: function(animate) {
            ...
            if (numOptions) {
                ...
                ranges.each(function(i, j) {
                    // valPercent get value of this handler
                    
                    // get value of nex handler
                    // if not last, get next
                    if(i == 0){
                        var nextPercent = (self.values(i) - min) / diff * 100;;
                        var valPercent = 0;
                    }
                    else if (0 < i && i < ranges.length-1){
                        var nextPercent = (self.values(i) - min) / diff * 100;
                        var valPercent = (self.values(i-1) - min) / diff * 100;
                    }
                    // else 100%
                ...
                var rangetexts = $('.ui-slider-range-text', this.element);
                // go through all ranges
                rangetexts.each(function(i, j) {
                    ...
                    if(i == 0){
                        ...
                        var nextPercent = (self.values(i) - min) / diff * 100;
                        var valPercent = Math.round((self.values(i-1) - min) / diff * 100);

                        /*math.round because valpercent will prodece unknown decimals*/
                        /*console.log(i+", self.value, "+self.values(i));
                        console.log(i+", next, "+nextPercent);
                        console.log(i+", val, "+valPercent);*/
                    }
                    // else 100%
                    else if(i == ranges.length-1){
                        var nextPercent = 100;
                    ...
                    // get range width
                    var rangePercent = (nextPercent - valPercent);
                    var posPercent = valPercent + rangePercent/2;
                    ...
                    if (self.orientation == 'horizontal') {
                        ...
                        $(this).children("h2").text(parseInt(rangePercent));
                    } else {
                        $(this).css({
                            bottom: posPercent + '%', 
                        });
